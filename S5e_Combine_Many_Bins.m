...
% This script creates new bins to account for combining the 31&32_I(N)MA and
% 31&32_C(N)MA individual subject averaged ERP waveforms into a single ERP
% set and for combining the 51_IN and 51_CN individual subject averaged ERP
% waveforms into a single ERP set.(It's a hard-coded script made from an
% existing script out of convenience, so it's not as streamlined/clean as
% one might prefer.)
...
for s=1:numsubjects
    ...
    for b=1
        ...
        % Load ERP sets
        subjectsetname = [subject '_recog_31&32_INMA&CNMA_ERPs_renamedbins_app.erp'];
        ...
        ERP = pop_binoperator( ERP, {['b15 = wavgbin(' non_flatlined_bins ') label Old NMA']});

        % Save the ERP with the new weighted average bin from the non-flatlined bins
        ERP = pop_savemyerp(ERP, 'erpname', [subject '_recog_31&32_INMA&CNMA_ERPs_renamedbins_app_NMAbin'], 'filename',[subject '_recog_31&32_INMA&CNMA_ERPs_renamedbins_app_NMAbin.erp'], 'filepath', subjectfolder, 'warning','off');
    end
    ...
    for b=2
        ...
        % Load ERP sets
        subjectsetname = [subject '_recog_31&32_IMA&CMA_ERPs_renamedbins_app.erp'];
        ERP = pop_loaderp ('filename', subjectsetname, 'filepath', subjectfolder);
        ...
        % Create a new weighted average bin from the non-flatlined bins using pop_binoperator()
        ERP = pop_binoperator( ERP, {['b15 = wavgbin(' non_flatlined_bins ') label Old MA']});

        % Save the ERP with the new weighted average bin from the non-flatlined bins
        ERP = pop_savemyerp(ERP, 'erpname', [subject '_recog_31&32_IMA&CMA_ERPs_renamedbins_app_MAbin'], 'filename',[subject '_recog_31&32_IMA&CMA_ERPs_renamedbins_app_MAbin.erp'], 'filepath', subjectfolder, 'warning','off');
    end
    ...
    for b=3
        ...
        % Load ERP sets
        subjectsetname = [subject '_recog_51_IN&CN_ERPs_app.erp'];
        ...
        % Create a new weighted average bin from the non-flatlined bins using pop_binoperator()
        ERP = pop_binoperator( ERP, {['b7 = wavgbin(' non_flatlined_bins ') label New']});

        % Save the ERP with the new weighted average bin from the non-flatlined bins
        ERP = pop_savemyerp(ERP, 'erpname', [subject '_recog_51_IN&CN_ERPs_renamedbins_app_Newbin'], 'filename',[subject '_recog_51_IN&CN_ERPs_renamedbins_app_Newbin.erp'], 'filepath', subjectfolder, 'warning','off');
    end
end